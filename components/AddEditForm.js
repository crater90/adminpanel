...
function AddEditForm({ data, edit }) {
    ...
    const deletePhoto = async (e) => {
        e.preventDefault();
        let tempRef = ref(storage, e.target.id);
        try {
            await deleteObject(tempRef)
            await updateDoc(doc(db, 'property', data.uniqueId), {
                photos: arrayRemove(e.target.id)
            })
            toast.success('Deleted image successfully')
        } catch (error) {
            toast.error('Not able to delete the image')
        }
    }

    return (
        <div>
            <Formik initialValues={data} onSubmit={async (values, { resetForm }) => {
                ...
                setImageArray([]);
                console.log('image state set to empty')
                resetForm();
            }}>
                {({ values }) => (
                    <Form>
                        ...
                        <div className='p-4 mb-2 border border-gray-200 shadow-sm bg-white rounded-lg grid grid-cols-6 gap-6'>
                            ...
                            <div className='relative col-span-6 md:col-span-2'>
                                ...
                                <div className="pointer-events-none absolute bottom-2.5 right-0 flex items-center px-2 text-gray-700">
                        ...
                        <div className='p-4 mb-2 border border-gray-200 shadow-sm rounded-lg bg-white grid grid-cols-6 gap-6'>
                            ...
                            {Object?.keys(values?.amenities).map(amenity => {
                                return (
                                    <div key={amenity} className='col-span-3 md:col-span-2 lg:col-span-1'>
                        ...
                        <div className='p-4 mb-2 border border-gray-200 shadow-sm rounded-lg bg-white grid grid-cols-6 gap-6'>
                            ...
                            {edit && data?.photos?.map(img => {
                                return <div key={img} className='relative'>
                                    <img src={img} key={img} className='h-40 w-40 rounded-md' />
                                    <XCircleIcon id={img} onClick={deletePhoto} className='mr-2 rounded-full mt-2 h-5 w-5 absolute top-0 right-0 bg-gray-100 cursor-pointer' />
                                </div>

                            })}

                            {edit && data.photos.length > 0 &&
                                <div className='col-span-6'>
                                    <p className='font-bold text-gray-500'>Note :<span className='font-normal'> Do not click on update after deleting images. Please visit this page again to see the changes. </span></p>
                                </div>
                        ...
                        <div className='py-2 flex items-center justify-center gap-4 text-white'>
                            <button type='reset' className='w-40 px-4 py-2 rounded shadow border border-blue-500 hover:bg-gray-100 text-gray-700 font-bold focus:outline-none'>{edit ? `Cancel` : `Reset`}</button>
                            <button type='submit' className='w-40 px-4 py-2 rounded shadow border border-blue-500 bg-blue-500 hover:bg-blue-700 text-white font-bold focus:outline-none'>{edit ? `Update` : `Submit`}</button>
                        </div>
                    </Form>
                )}
